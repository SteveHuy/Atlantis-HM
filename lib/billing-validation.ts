...
export const makePaymentSchema = z.object({
  ...
  paymentMethod: z.enum(['Credit Card', 'ACH Transfer'] as const, {
  ...
  nameOnCard: z.string().optional().refine((val) => {
    if (!val) return true;
    return val.length >= 1 && val.length <= 100;
  }, 'Name on card must be between 1-100 characters'),
  // ACH Fields
...
export const paymentPlanSchema = z.object({
  ...
  startDate: z.string().refine((val) => val && val.length > 0, 'Start date is required')
});

// Process Payment Validation
export const processPaymentSchema = z.object({
  paymentId: z.string().refine((val) => val && val.length > 0, 'Payment ID is required'),
...
export const remittanceReconciliationSchema = z.object({
  remittanceId: z.string().refine((val) => val && val.length > 0, 'Remittance ID is required'),
  claimId: z.string().refine((val) => val && val.length > 0, 'Claim ID is required'),
...
export const billingUpdateSchema = z.object({
  transactionId: z.string().refine((val) => val && val.length > 0, 'Transaction ID is required'),
